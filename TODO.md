...
- [x] **T036 · Test Fix · P1: Update validation error assertions in card review API tests**
    - **Context:** Test failures in card review API tests are caused by outdated validation error message assertions discovered during T035 implementation.
    - **Action:**
        1. Locate all card review API tests asserting on validation error messages.
        2. Update each assertion to match the current error format produced by the API.
        3. Where possible, match on key substrings or error codes instead of full message strings for resilience.
    - **Done-when:**
        1. All identified test assertions have been updated.
        2. The modified tests compile and pass locally for the previously failing cases.
    - **Depends-on:** [T035]
    - **Estimated Story Points:** 1
...
- [x] **T037 · Refactor · P2: Introduce helper for robust validation error matching**
    ...
    - **Action:**
        ...
        2. Replace direct string-equality assertions in card review API tests with this helper.
        3. Document the helper's usage pattern in the test suite.
    - **Done-when:**
        1. All relevant tests use the new helper.
...
- [ ] **T038 · Test · P2: Add regression tests for edge-case validation errors**
    ...
    - **Action:**
        1. Review card review validation rules and identify under-tested scenarios (e.g., missing fields, invalid values).
        2. Write new table-driven tests to cover each edge case, leveraging the validation-error helper from T037.
    - **Done-when:**
        1. New tests cover all identified edge cases.
        2. All tests pass, and coverage meets project standards.
    - **Depends-on:** [T037]
    - **Estimated Story Points:** 1

- [ ] **T039 · Chore · P1: Run full test suite and CI to confirm stability**
    - **Context:** Validate that updates resolve failures and do not introduce new issues.
    - **Action:**
        1. Execute the full local test suite (`make test`, integration tests).
        2. Trigger CI pipeline and verify all stages (lint, format, unit tests, integration tests) pass.
        3. Investigate and remedy any remaining failures.
    - **Done-when:**
        1. All tests pass locally and in CI.
        2. No new warnings or failures are present.
    - **Depends-on:** [T038]
    - **Estimated Story Points:** 1

- [ ] **T040 · Chore · P1: Mark T035 as completed and close related issue**
    - **Context:** With validation-test failures resolved, the original T035 work is now fully done.
    - **Action:**
        1. Update `TODO.md` to change T035 from `[ ]` to `[x]`.
        2. Add a note summarizing "Resolved validation-error message expectations in card review API tests."
        3. If applicable, close any linked issue or ticket for T035.
    - **Done-when:**
        1. T035 is marked as completed in `TODO.md`.
        2. Associated issue/ticket is closed or documented as resolved.
    - **Depends-on:** [T039]
    - **Estimated Story Points:** 1
