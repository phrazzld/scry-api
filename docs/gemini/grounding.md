...
The following example shows how to configure a model to use Google Search retrieval:
...
Here are examples of some prompts and their prediction scores (note that prediction scores are assigned by Gemini and can vary over time depending on several factors):
...
- **Threshold**: In your API request, you can specify a dynamic retrieval configuration with a threshold. The threshold is a floating-point value in the range [0,1] and defaults to 0.3. If the threshold value is zero, the response is always grounded with Google Search. For all other values of threshold:
