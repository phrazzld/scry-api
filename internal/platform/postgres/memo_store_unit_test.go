...
func TestNewPostgresMemoStore(t *testing.T) {
	tests := []struct {
		name        string
		db          store.DBTX
		logger      *slog.Logger
		expectPanic bool
		check       func(t *testing.T, store *PostgresMemoStore)
	}{
		{
			name:        "nil_db_panics",
			db:          nil,
		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectPanic {
				assert.Panics(t, func() {
					NewPostgresMemoStore(tt.db, tt.logger)
				})
				return
			}

			store := NewPostgresMemoStore(tt.db, tt.logger)
			if tt.check != nil {
				tt.check(t, store)
			}
		})
...
func TestPostgresMemoStore_WithTx(t *testing.T) {
	...
	originalDB := &sql.DB{}
	logger := slog.Default()
	store := NewPostgresMemoStore(originalDB, logger)
