...
func TestTaskRunner_Recover(t *testing.T) {
	...
	err := store.SaveTask(context.Background(), pendingTask)
	require.NoError(t, err, "Failed to save pending task")
...
func TestTaskRunner_StuckTasks(t *testing.T) {
	...
	err := store.SaveTask(context.Background(), stuckTask)
	require.NoError(t, err, "Failed to save stuck task")

	err = store.UpdateTaskStatus(context.Background(), stuckTask.ID(), TaskStatusProcessing, "")
	require.NoError(t, err, "Failed to update stuck task status")
	...
	// Start the runner
	runnerStartErr := runner.Start()
	require.NoError(t, runnerStartErr)
